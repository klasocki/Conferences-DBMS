...
SELECT ClientID,
       ...
       (SELECT Cancelled
        ...
        WHERE ID = ReservationId) as isCancelled,
       ConferenceName,
       ConferenceID,
...
SELECT TOP 10 ID,
              ...
              (SELECT SUM(PlaceCount)
               ...
               WHERE ClientID = Clients.ID
                 AND DayReservations.Cancelled = 0) as TotalPlacesBooked
FROM Clients
...
SELECT ClientID,
       ...
       (SELECT SUM(Amount)
        FROM Payments
               JOIN ConferenceReservations CR on Payments.ConferenceReservationID = CR.ID
        WHERE ReservationDetails.ClientID = CR.ClientID) - SUM(PriceToPayForEntries + PriceToPayForWorkshops)
                                                          as Balance
